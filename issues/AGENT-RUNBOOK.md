# AGENT RUNBOOK — PDF Password Manager (autonomous fix pass)

**You are an autonomous coding agent.** When you receive this file, START IMMEDIATELY and work through the
worklist below **one card at a time, in order, without waiting for the human between cards.** Apply → verify →
commit → next, until every runnable card is DONE or you hit a STOP condition. The full task cards are embedded
after the worklist (## Prompt 1 … ## Prompt 30); each card has verbatim "Locate" (current code) and "Change"
(new code), tests, and acceptance criteria.

> This runbook runs on the operator's own machine and your own checkout of the repo. Operate in the repository
> root (a Flutter app). Treat the embedded cards as the source of truth.

## Environment
- **App / package:** `com.passwordpdf.passwordpdf_manager` (Flutter, Android-first).
- **Toolchain:** Flutter 3.38.6 / Dart 3.10.7 via **FVM** → use `fvm flutter …` (fallback `flutter …`).
- **Card source:** this file (embedded below) and `issues/pdf-password-manager-implementation-handbook.md`.
- **Device + ADB (attached physical device):** adb is at `D:\idm\platform-tools-latest-windows\adb.exe`.
  Verify before any device test: `& "D:\idm\platform-tools-latest-windows\adb.exe" devices` → expect exactly one line ending in `device`.
  (Optionally add `D:\idm\platform-tools-latest-windows` to PATH so plain `adb` works.)

## The per-card loop (repeat for every worklist item, in order)
1. **Idempotency check.** If the card's "Change" code is ALREADY present in the file (e.g. Task 2 may already be done), mark it `DONE (already applied)` and move on. Do not re-apply.
2. **Locate.** Find each "Locate" block by EXACT string match in the named file. Match on the CODE, **not** line numbers (they shift). If a block is **not found character-for-character → STOP** (see STOP conditions); never approximate.
3. **Change.** Apply the "Change" verbatim. If the card says the block occurs at multiple places, change ALL of them. For "Insert after", insert (don't replace). Create any `new …` file or `test/…` file the card gives; add the exact `import` / `pubspec.yaml` lines; run `fvm flutter pub get` if pubspec changed.
4. **Verify (code).** Run `fvm flutter analyze` — it MUST be clean for the touched file(s). Run any unit test the card provides (`fvm flutter test <path>`). If analyze or a test fails → **revert this card's changes and STOP.**
5. **Commit.** `git add -A && git commit -m "Task <N>: <title>"`. One card = one commit.
6. **Record** a one-line result and proceed to the next card. Do not pause for confirmation.

## Device smoke-test (at each PHASE boundary — not per card)
After finishing all cards in a phase, build & install once and run that phase's "Manual" checks on the device:
- `fvm flutter build apk --debug`
- `& "D:\idm\platform-tools-latest-windows\adb.exe" install -r build/app/outputs/flutter-apk/app-debug.apk`
- Launch: `& "D:\idm\platform-tools-latest-windows\adb.exe" shell monkey -p com.passwordpdf.passwordpdf_manager -c android.intent.category.LAUNCHER 1`
- Watch logs: `& "D:\idm\platform-tools-latest-windows\adb.exe" logcat -s flutter`
Perform the "Manual" steps listed in the cards of that phase and confirm the expected results. If a check fails → STOP and report the card + observed behavior. If no device is connected when a smoke-test is due, finish the code+analyze for the phase, then STOP and ask the operator to attach the device.

## Worklist (apply in THIS order — the embedded ## Prompt N blocks match these)
1. Task 2 — resilient library load  ·  2. Task 5 — guard device deletion  ·  3. Task 7 — folder cycle check
4. Task 8 — orElse getPhysicalPathForFolder  ·  5. Task 6 — IF NOT EXISTS migrations  ·  6. Task 3 — sync curated-column preserve
7. Task 4 — overwrite-import temp-then-swap  ·  8. Task 9 — pagination/multi-delete try/finally  ·  9. Task 10 — Open-With intent guard
10. Task 1 — harden self-update  ·  11. Task 13 — cleanupUpdateFile glob  ·  12. Task 29 — collapse _performUpdate
...
Tasks **15, 16, 17, 18, 19** (the AES crypto migration), **25** (scope storage permissions / drop MANAGE_EXTERNAL_STORAGE — mandatory for sideloaded scanner), **26** (PDF-tools fidelity — superseded by Part 2 in-place fix), **27** (export remove-password — cross-file dependency), **30 & 31** (store refactor brief). These are excluded from the worklist on purpose.
...
On STOP, report: the Task #, the exact problem, the file, and what you tried. Then wait for the operator.

## Non-negotiable rules
Match code not line numbers · one card = one commit · stay strictly in scope (change nothing the card doesn't name) · never `print` (use the existing `LoggingService`) · production app with real saved passwords — never delete/overwrite stored data except via a card's verification step · don't reformat/refactor unrelated code.

## Final report (when the worklist ends or you STOP)
A table: Task # → DONE / DONE(already) / SKIPPED(senior) / STOPPED(reason), the commits made, and any STOP details.

---

**START NOW** with worklist item 1 and proceed autonomously through item 30. The embedded cards follow.
...
### Prompt 24 / Task 26 — SUPERSEDED (senior owns PDF fidelity)

This card's `Change` blocks route through `importPageRange`, which
SENIOR-RUNBOOK.md Part 2 proved is the visual flatten (Syncfusion 32.x
has no true page import). The current `lib/services/pdf_tools_service.dart`
already ships the correct in-place fixes for `removePassword` and
`splitPdf`. Applying Prompt 24 as-written would REGRESS those.

The only remaining Part-2 work is `reorderPages` → `pages.reArrange()`,
which is owned by `PENDING-AGENT-RUNBOOK.md` Phase 7 (Card 7.1).

**Do NOT apply this card.** See SENIOR-RUNBOOK.md Part 2.
